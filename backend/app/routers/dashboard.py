...
def get_dashboard(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    read_books = db.query(Book).filter(Book.user_id == current_user.id, Book.read.is_(True))

    total_read = read_books.count()
    avg_rating = db.query(func.avg(Book.rating)).filter(
        Book.user_id == current_user.id, Book.read.is_(True), Book.rating.isnot(None)
    ).scalar() or 0
    pages_read = db.query(func.coalesce(func.sum(Book.pages), 0)).filter(
        Book.user_id == current_user.id, Book.read.is_(True)
    ).scalar()

    genre_rows = (
        db.query(Book.genre, func.count(Book.id))
        .filter(Book.user_id == current_user.id, Book.read.is_(True), Book.genre.isnot(None))
        .group_by(Book.genre)
    ...
    rating_rows = (
        ...
        .filter(Book.user_id == current_user.id, Book.read.is_(True), Book.rating.isnot(None))
    ...
    month_rows = (
        ...
        .filter(
            ...
            extract("year", Book.end_date) == year,
        )
